...
export function selectWeekForEditing(weekID) {
  const selected = state.weeks.find((week) => Number(week.id) === Number(weekID));
  state.weekDraft = weekDraftFromWeek(selected || null);
  render();
}

export function createWeekDraft() {
  state.weekDraft = weekDraftFromWeek(null);
  render();
}
...
const newTestamentVideoFiles = {
  '01': 'prefaceA.mp4', '02': 'prefaceB.mp4', '03': 'mattewA.mp4', '04': 'mattewB.mp4',
  '05': 'actsA.mp4', '06': 'actsB.mp4', '07': 'markA.mp4', '08': 'markB.mp4',
  '09': 'lukeA.mp4', '10': 'lukeB.mp4', '11': 'johnA.mp4', '12': 'johnB.mp4',
  '13': 'gospelsA.mp4', '14': 'gospelsB.mp4', '16': 'PaulLetterB.mp4',
  '20': 'thessaloniansD.mp4', '21': 'corinthiansA.mp4', '22': 'corinthiansB.mp4',
  '23': 'corinthians2A.mp4', '24': 'corinthians2B.mp4', '25': 'corinthians2C.mp4', '26': 'corinthians2D.mp4',
  '27': 'galatiansA.mp4', '28': 'galatiansB.mp4', '29': 'galatiansC.mp4',
  '30': 'romansA.mp4', '31': 'romansB.mp4', '32': 'colossians.mp4', '33': 'ephesians.mp4',
  '34': 'philippiansA.mp4', '35': 'philippiansB.mp4', '36': 'philemon.mp4',
  '38': 'timothy1.mp4', '39': 'titus.mp4', '40': 'timothy2.mp4',
  '41': 'hebrewsA.mp4', '42': 'hebrewsB.mp4', '43': 'Peter1.mp4', '44': 'Peter2.mp4',
  '45': 'james.mp4', '46': 'johnLetter1A.mp4', '47': 'johnLetter1B.mp4',
  '48': 'johnLetter23.mp4', '49': 'jude.mp4', '50': 'revelationA.mp4', '51': 'revelationB.mp4',
};

function externalNewTestamentVideoURL(title, originalURL = '') {
  const configured = state.siteConfig?.site_info?.video_base_url || state.siteConfig?.video_base_url;
  const base = String(configured || 'https://mouss.synology.me:1050/Newtestament/').replace(/\/+$/, '');
  let searchable = `${title || ''} ${originalURL || ''}`;
  try { searchable = decodeURIComponent(searchable); } catch (_) { /* keep original text */ }
  const lesson = searchable.match(/(?:^|[^0-9])(0[1-9]|[1-4][0-9]|5[01])(?:[^0-9]|$)/)?.[1];
  if (lesson && newTestamentVideoFiles[lesson]) return `${base}/${newTestamentVideoFiles[lesson]}`;
  if (/^\/Newtestament\//i.test(originalURL)) return `${base}/${String(originalURL).replace(/^\/Newtestament\//i, '')}`;
  return originalURL;
}
...
function normalizeVideoItem(item) {
  if (typeof item === 'string') {
    ...
    if (parts.length >= 2) return { title: parts[0], url: externalNewTestamentVideoURL(parts[0], parts.slice(1).join('|')) };
    return { title: raw, url: externalNewTestamentVideoURL(raw) };
  }
  if (!item || typeof item !== 'object') return null;
  const title = String(item.title || item.name || item.video || '').trim();
  const url = String(item.url || item.href || item.path || '').trim();
  return title || url ? { title: title || url, url: externalNewTestamentVideoURL(title, url) } : null;
}
...
function normalizeWeekVideos(plan) {
  ...
  return title || url ? [{ title: title || url, url: externalNewTestamentVideoURL(title, url) }] : [];
}
