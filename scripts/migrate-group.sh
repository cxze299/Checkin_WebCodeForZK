...
用法:
  ...
  NAME_MAP_PATH=/path/to/name-map.json \\
...
validate() {
  ...
  if [ -n "$NAME_MAP_PATH" ] && [ ! -f "$NAME_MAP_PATH" ]; then
    echo "姓名映射文件不存在: $NAME_MAP_PATH" >&2
    exit 1
  fi
...
prepare_inputs() {
  ...
  if [ -n "$NAME_MAP_PATH" ]; then
    cp "$(abs_path "$NAME_MAP_PATH")" "$TMP_INPUT_DIR/name-map.json"
    NAME_MAP_IN_CONTAINER="/workspace/.tmp/migration-inputs-${GROUP_CODE}/name-map.json"
  else
    NAME_MAP_IN_CONTAINER=""
  fi
}
...
run_cli() {
  ...
  if [ -n "$NAME_MAP_IN_CONTAINER" ]; then
    args+=("--name-map" "$NAME_MAP_IN_CONTAINER")
  fi
  docker run --rm \
